...
function computeDefaults(schema, parentDefaults) {
  ...
  var schemaIndex = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
  var isNestedDiscriminator = arguments[4];
  ...
  if (isObject(defaults) && isObject(schema.default)) {
    ...
    defaults = {
      $$__case: schemaIndex,
      $$__case_of: "oneOf",
      value: computeDefaults(schema.oneOf[schemaIndex], undefined, definitions, schemaIndex, true)
    };
  } else if ("anyOf" in schema) {
    defaults = {
      $$__case: schemaIndex,
      $$__case_of: "anyOf",
      value: computeDefaults(schema.anyOf[schemaIndex], undefined, definitions, schemaIndex, true)
    };
  }
...
function getDefaultFormState(_schema, formData) {
  ...
  var defaults = computeDefaults(schema, _schema.default, definitions, schemaIndex);
...
function retrieveSchema(schema) {
  ...
  if (schema.hasOwnProperty("$ref")) {
    ...
    schema.allOf = schema.allOf.map(function (schema) {
      return retrieveSchema(schema, definitions, formData);
    });
    return schema;
  } else {
