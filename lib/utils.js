...
function getStructure(modelName, structures) {
  var structure = structures.find(function (struct) {
    return struct.Name === modelName || mergeStructure.OriginalName === modelName;
  });
...
function mergeStructure(schema, structure, linkMapper) {
  if (structure && structure.Fields) {
    structure.Fields.forEach(function (field) {
      var property = schema.properties[field.Name];
      if (property) {
        if (property.type === "array") {
          schema.properties[field.Name].items = _extends({}, property.items, {
            typeCombinatorTypes: field.TypeCombinatorTypes
          });
        } else {
          schema.properties[field.Name] = _extends({}, property, generateAdditionalProperties(field, linkMapper));
        }
      }
    });
  }
