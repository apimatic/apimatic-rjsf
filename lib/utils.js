...
exports.isMultipleSchema = exports.classPrefix = undefined;
  ...
  if (isObject(newFormData)) {
    if (checkDiscriminator(newFormData)) {
      ...
      for (var key in newFormData) {
        newFormData = _extends({}, newFormData, _defineProperty({}, key, unwrapFormData(newFormData[key])));
      }
    }
  }
...
function computeDefaults(schema, parentDefaults) {
  ...
  var dxInterface = arguments[3];
  var _dxInterface$definiti = dxInterface.definitions,
      definitions = _dxInterface$definiti === undefined ? {} : _dxInterface$definiti;
    ...
    case "object":
      {
        defaults = Object.keys(schema.properties || {}).reduce(function (acc, key) {
          acc[key] = computeDefaults(schema.properties[key], (defaults || {})[key], undefined, dxInterface);
          return acc;
        }, {});
        break;
      }
...
function getStructure(modelName, structures) {
  var linkTo = "$m/" + modelName;
  var structure = structures.find(function (struct) {
    return struct.LinkTo === linkTo;
  });
...
function getDescription(type) {
  return type.Description ? type.Description.trim() !== '-' ? type.Description : undefined : undefined;
}
    ...
    if (structure) {
      ...
      if (structure.Fields) {
        structure.Fields.forEach(function (field) {
          var property = selectedSchema.properties[field.Name];
          if (property) {
            if (property.type === "array") {
              refSchema.allOf[selectedIndex].properties[field.Name].items = _extends({}, property.items, { typeCombinatorTypes: field.TypeCombinatorTypes });
            } else {
              refSchema.allOf[selectedIndex].properties[field.Name] = _extends({}, property, generateAdditionalProperties(field, linkMapper));
            }
          }
        });
      }
...
function retrieveSchema(schema) {
  ...
  var dxInterface = arguments[2];
  var _dxInterface$definiti2 = dxInterface.definitions,
      definitions = _dxInterface$definiti2 === undefined ? {} : _dxInterface$definiti2,
      _dxInterface$structur = dxInterface.structures,
      structures = _dxInterface$structur === undefined ? [] : _dxInterface$structur,
      linkMapper = dxInterface.linkMapper;


  if (schema.hasOwnProperty("$ref")) {
    var modelName = schema["$ref"].replace("ModelSchemas#/", "");
    ...
    return retrieveSchema(_extends({}, $refSchema, localSchema), formData, dxInterface);
...
function withExactlyOneSubschema(schema, formData, dependencyKey, oneOf, dxInterface) {
  ...
  var validSubschemas = oneOf.filter(function (subschema) {
    ...
    if (conditionPropertySchema) {
      ...
      var _validateFormData = (0, _validate2.default)(formData, conditionSchema, undefined, undefined, undefined, definitions),
...
function toIdSchema(schema, id) {
  ...
  if ("$ref" in schema) {
    // clear
    var _schema = retrieveSchema(schema, formData, dxInterface);
...
var classPrefix = exports.classPrefix = "rjsf-";
  ...
  for (var i = 0; i < options.length; i++) {
    ...
    if (option.properties) {
      ...
      if (option.anyOf) {
        ...
        var shallowClone = _objectWithoutProperties(option, []);
...
var isMultipleSchema = exports.isMultipleSchema = function isMultipleSchema(schema) {
  return !!(schema && (schema.hasOwnProperty("oneOf") || schema.hasOwnProperty("anyOf")));
};

function classNames(classObj) {
  if ((typeof classObj === "undefined" ? "undefined" : _typeof(classObj)) !== "object") {
    return;
  }
