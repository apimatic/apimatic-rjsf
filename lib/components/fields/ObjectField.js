...
var READONLY_INFO_MESSAGE = "All parameters of this object are Response Only. Response Only means that it may be sent as a part of the response but should not be sent as part of the request.";
...
var ObjectField = function (_Component) {
  ...
  _createClass(ObjectField, [{
    ...
    value: function render() {
      ...
      console.log(" MY SCHEMA  -->>", (0, _utils.retrieveSchema)(this.props.schema, formData, dxInterface));
      var title = name;
    ...
    value: function renderObject(templateProps) {
      ...
      return _react2.default.createElement(
        ...
        null,
        _react2.default.createElement(Template, newProps)
      );
    }
  }]);
...
function checkSchemaForReadOnly(schema) {
  ...
  var newPROP = Object.values(properties).every(function (sc) {
    return sc.readOnly;
  });
  console.log("checkSchemaForReadOnly --->", newPROP);

  /* let propObject = [];
  let count = 0;
  // console.log("checkSchemaForReadOnly --->", properties);
  if (properties) {
    propObject = Object.keys(properties);
    propObject.map(item => {
      properties[item].readOnly && count++;
    });
  } */
  return newPROP;
}
